""" tworzenie skróconych wersji biogramów PSB - miejscowości """
import os
import time
...
def short_version(text:str) -> str:
    """ proste skracanie biogramów, zdania istotne dla miejscowości """
    result = ''
    select_data = []
    ...
    # ze dalszych zdań tylko takie wskazujące na związek z ważnymi osobami
    # czyli zawierające encję typu persName
...
if __name__ == '__main__':
    ...
    for nr_tomu in range(1, 52):
        tom = 'tom_' + str(nr_tomu).zfill(2)
        data_folder = Path("..") / "data_psb" / "full" / tom
        data_file_list = data_folder.glob('*.txt')
        if not os.path.isdir(Path("..") / 'data_psb' / 'short_places' / tom ):
            os.mkdir(Path("..") / 'data_psb' / 'short_places' / tom )
        ...
        for data_file in data_file_list:
            ...
            output_path = Path("..") / 'data_psb' / 'short_places' / tom / data_file_name
