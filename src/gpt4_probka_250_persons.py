""" analiza próbki 250 biogramów - ważne osoby dla postaci """
import os
import sys
import json
import time
...
# na koncie IHPAN openai zwraca info że jest tylko 10000 na minutę!
...
def test_if_relative(item:dict) -> bool:
    """ test czy znaleziona osoba nie jest jednak krewnym"""
...
if __name__ == '__main__':
    ...
    prompt_path = Path("..") / "prompts" / "person_persons.txt"
    ...
    for data_file in data_file_list:
        ...
        output_path = Path("..") / 'output_json_250' / 'persons' / data_file_name.replace('.txt','.persons.json')
        if os.path.exists(output_path):
            print(f'Plik {data_file_name.replace(".txt",".persons.json")} z wynikiem przetwarzania już istnieje, pomijam...')
            continue

        # text biogramu jest dzielony
        ...
        # weryfikacja czy w znaleziskach nie ma krewnych + usuwanie z wyników
        tmp_llm_dict = llm_dict.copy()
        for llm_item in tmp_llm_dict:
            if test_if_relative(llm_item):
                llm_dict.remove(llm_item)
        ...
        # obsługa limitów wyłączona, przetwarzanie długich tekstów przy obecnej
        # wydajności API z OpenAI trwa na tyle długo, że praktycznie nie ma szans
        # na przekroczenie limitów, w trakcie testów i pracy zdarzyło się raz...
